...
async function getUsernameFromDockerConfig(): Promise<string | null> {
          ...
          const answers = await inquirer.default.prompt([
            {
              ...
              choices: usernames.map((item) => ({
                name: `${item.username} (${item.registry})`,
                value: item.username,
              })),
            },
          ]);
...
async function main(): Promise<void> {
  try {
    ...
    let usernameFromDockerConfig: string | null;

    username = getDockerInfo().username;

    if (username === null || username === undefined) {
      usernameFromDockerConfig = await getUsernameFromDockerConfig();
      username = usernameFromDockerConfig;
    }
