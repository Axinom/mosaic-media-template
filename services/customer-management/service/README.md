...
This is the backend service with a focus on user management. It uses PostgreSQL
...
user service has a dedicated script to generate working ingest document files:
...
- Request existing encoding processing profiles from configured encoding service
  URL
- Request existing genres from the user service database
...
- Because the script also makes requests to encoding service API and user
