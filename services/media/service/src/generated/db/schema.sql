...
CREATE TRIGGER _100_timestamps BEFORE UPDATE ON app_public.reviews FOR EACH ROW WHEN ((old.* IS DISTINCT FROM new.*)) EXECUTE PROCEDURE ax_utils.tg__timestamps();
...
CREATE TRIGGER _200_username BEFORE UPDATE ON app_public.reviews FOR EACH ROW WHEN ((old.* IS DISTINCT FROM new.*)) EXECUTE PROCEDURE ax_utils.tg__username();
...
CREATE TRIGGER _200_username_before_insert BEFORE INSERT ON app_public.reviews FOR EACH ROW EXECUTE PROCEDURE ax_utils.tg__username();
...
CREATE TRIGGER _500_gql_reviews_deleted BEFORE DELETE ON app_public.reviews FOR EACH ROW EXECUTE PROCEDURE ax_utils.tg__graphql_subscription('ReviewDeleted', 'graphql:reviews', 'id');
...
CREATE TRIGGER _500_gql_reviews_inserted AFTER INSERT ON app_public.reviews FOR EACH ROW EXECUTE PROCEDURE ax_utils.tg__graphql_subscription('ReviewCreated', 'graphql:reviews', 'id');
...
CREATE TRIGGER _500_gql_reviews_updated AFTER UPDATE ON app_public.reviews FOR EACH ROW EXECUTE PROCEDURE ax_utils.tg__graphql_subscription('ReviewChanged', 'graphql:reviews', 'id');
...
CREATE POLICY reviews_authorization ON app_public.reviews USING ((ax_utils.user_has_permission('REVIEWS_VIEW,REVIEWS_EDIT,ADMIN'::text) AND (1 = 1))) WITH CHECK ((ax_utils.user_has_permission('REVIEWS_EDIT,ADMIN'::text) AND (1 = 1)));
...
CREATE POLICY reviews_authorization_delete ON app_public.reviews AS RESTRICTIVE FOR DELETE USING (ax_utils.user_has_permission('REVIEWS_EDIT,ADMIN'::text));
