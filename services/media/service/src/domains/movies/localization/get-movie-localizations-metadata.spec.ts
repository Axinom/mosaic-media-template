...
import {
  ...
  GqlMovieLocalization,
} from './get-movie-localizations-metadata';
...
describe('getMovieLocalizationsMetadata', () => {
  ...
  const createPublishObject = (localizations: GqlMovieLocalization[]) => {
    return {
      data: {
  ...
  it('Validation returns an error -> returned localizations is undefined, returned validations are formatted', async () => {
    ...
    // Act
    const { result, validation } = await getMovieLocalizationsMetadata(
      config,
      mockedString,
      mockedString,
    );
  ...
  it('Returned localizations have properties that are different from expected -> error thrown', async () => {
    ...
    const error = await rejectionOf(
      getMovieLocalizationsMetadata(config, mockedString, mockedString),
    );
