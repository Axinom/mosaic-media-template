...
import {
  ...
  GqlCollectionLocalization,
} from './get-collection-localizations-metadata';
...
describe('getCollectionLocalizationsMetadata', () => {
  ...
  const createPublishObject = (localizations: GqlCollectionLocalization[]) => {
    return {
      data: {
  ...
  it('Validation returns an error -> returned localizations is undefined, returned validations are formatted', async () => {
    ...
    // Act
    const { result, validation } = await getCollectionLocalizationsMetadata(
      config,
      mockedString,
      mockedString,
    );
  ...
  it('Returned localizations have properties that are different from expected -> error thrown', async () => {
    ...
    const error = await rejectionOf(
      getCollectionLocalizationsMetadata(config, mockedString, mockedString),
    );
