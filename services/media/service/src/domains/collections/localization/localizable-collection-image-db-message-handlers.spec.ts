...
describe('Localizable Collection Image DB trigger events', () => {
  ...
  const createMessage = (
    ...
    > & {
      metadata?: Partial<
        TypedTransactionalMessage<LocalizableCollectionImageDbEvent>['metadata']
      >;
    } as unknown as TypedTransactionalMessage<LocalizableCollectionImageDbEvent>;
  ...
  describe('LocalizableCollectionImageDeletedDbMessageHandler', () => {
    ...
    it('collection cover is deleted and collection still exists -> upsert message data is sent', async () => {
      ...
      vi.spyOn(handler, 'collectionIsDeleted').mockImplementation(
        async () => false,
      );
