...
import * as localizationMetadata from '../localization/get-episode-localizations-metadata';
...
describe('publishingEpisodeProcessor', () => {
  ...
  describe('aggregator', () => {
    ...
    it('full metadata episode -> valid result with mocked warnings and errors', async () => {
      ...
      const localizations: EpisodeLocalization[] = [
        {
          title: 'source title',
  ...
  describe('commonPublishValidator and validator', () => {
      ...
      const result = await commonPublishValidator(
        {
          result: {
            ...
            licenses: [{}],
            genre_ids: ['test'],
            images: [{ type: 'EPISODE_COVER_1x1', width: 0, height: 0, path: '' }],
            videos: [
              {
                type: 'MAIN',
                is_protected: false,
    ...
    it('episode right after creation -> specific errors', async () => {
      ...
      expect(result).toEqual([
        ...
        {
          context: "VIDEO",
          message: "Intro start time cue point must be less than the video length.",
          severity: "ERROR",
        },
        {
          context: "VIDEO",
