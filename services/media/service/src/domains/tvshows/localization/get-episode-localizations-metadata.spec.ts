...
import {
  ...
  GqlEpisodeLocalization,
} from './get-episode-localizations-metadata';
...
describe('getEpisodeLocalizationsMetadata', () => {
  ...
  const createPublishObject = (localizations: GqlEpisodeLocalization[]) => {
    return {
      data: {
  ...
  it('Validation returns an error -> returned localizations is undefined, returned validations are formatted', async () => {
    ...
    // Act
    const { result, validation } = await getEpisodeLocalizationsMetadata(
      config,
      mockedString,
      mockedString,
    );
  ...
  it('Returned localizations have properties that are different from expected -> error thrown', async () => {
    ...
    const error = await rejectionOf(
      getEpisodeLocalizationsMetadata(config, mockedString, mockedString),
    );
