...
import {
  getTvshowGenreLocalizationsMetadata,
  GqlTvshowGenreLocalization,
} from './get-tvshow-genre-localizations-metadata';
...
describe('getTvshowGenreLocalizationsMetadata', () => {
  ...
  const createPublishObject = (localizations: GqlTvshowGenreLocalization[]) => {
    return {
      data: {
  ...
  it('Validation returns an error -> returned localizations is undefined, returned validations are formatted', async () => {
    ...
    // Act
    const { result, validation } = await getTvshowGenreLocalizationsMetadata(
      config,
      mockedString,
      mockedString,
      'Drama',
