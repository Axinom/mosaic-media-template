...
--! Hash: sha1:bd8d704a797f4f5129748fb2e76477d7ce2f13ad
...
{
  "body": [
    "SELECT ax_define.define_logical_replication_slot(slotName => '${1:replication_slot_name_or_placeholder}',",
    "                                                 skipIfMatches => '${2:^.*(_t|_shadow)$}');"
  ],
  "description": [
    "An idempotent wrapper for pg_create_logical_replication_slot.\n",
    "The replication slot is created on a PostgreSQL server level and has a unique name across all databases.\n",
    "It must also be created after the publications which will be utilized together with the slot to watch for table changes.\n",
    "Because of the slot name uniqueness, special care must be taken to avoid other databases like graphile-migrate shadow database or test database(s) to create the slot with the same name for themselves.\n",
    "For this reason, second parameter skipIfMatches is expected to skip creating the slot if current database name matches a certain pattern.\n",
    "Plugin used by default is `pgoutput`. It can be overridden using the third parameter."
  ]
}
snippet-*/
CREATE OR REPLACE FUNCTION ax_define.define_logical_replication_slot(slotName TEXT, skipIfMatches TEXT, pluginName TEXT DEFAULT 'pgoutput'::text) RETURNS void AS $$
DECLARE
  databaseName TEXT;
BEGIN
