--! Previous: sha1:7d195977395ec5e4a14dbf48def809c501fee122
--! Hash: sha1:ac2d5bf7add3062937fb93b527ff39f0f93b9453
...
/*-snippet
{
  "body": [
    ...
    "-- - Use `enableSubscriptions` and `.addHookPluginFactory(subscriptionAuthorizationHookFactory)` and on the `PostgraphileOptionsBuilder`.",
...
{
  "body": [
    "SELECT ax_define.expose_enum_endpoint('${1:enum_name}', '${2:app_public}');"
  ],
  "description": [
    "Exposes PostgreSQL enum values using a custom GraphQL query endpoint.\n",
    "Consider switching to using tables for enum values:",
    "https://www.graphile.org/postgraphile/enums/",
    "Using enum tables will remove the need to expose enum endpoints."
  ]
}
snippet-*/
...
{
  "body": [
    "SELECT ax_define.unbind_enum_type_from_table('${1:column_name}', '${2:table_name}', '${3:app_public}', '${4:DEFAULT_VALUE}');"
  ],
  "description": [
    "Alters the table by switching the type of a specified column from enum to text or text[].",
    "The fourth parameter is optional. Do not define it if the enum column has no default value.",
    "If the column type is an array - pass true as a fifth parameter (default is false).",
    "Execution is skipped if the column does not exist.\n",
    "This is one of the functions that is needed to migrate from using enum types to enum tables.",
    "Use 'ax-migrate-enum-type-to-table' for a full migration code example."
  ]
}
snippet-*/
...
/*-snippet
{
  "body": [
      ...
      "  '${5:app_public}',",
      "  ':${6:DATABASE_LOGIN}',",
      "  '{\"${2:DEFAULT_VALUE}\",\"${3:OTHER_VALUE}\",\"${4:ANOTHER_VALUE}\"}',",
      "  '{\"${2/(?:([A-Z])([A-Z]+))*(_)?([A-Z]+)/${1:/capitalize}${2:/downcase}${3:+ }${4:/downcase}/g}\",\"${3/(?:([A-Z])([A-Z]+))*(_)?([A-Z]+)/${1:/capitalize}${2:/downcase}${3:+ }${4:/downcase}/g}\",\"${4/(?:([A-Z])([A-Z]+))*(_)?([A-Z]+)/${1:/capitalize}${2:/downcase}${3:+ }${4:/downcase}/g}\"}');"
  ],
  "description": [
    "Drops the enum type and creates an enum table with the same name.",
    "Must be called after 'unbind_enum_type_from_table' call.\n",
...
{
  "body": [
      "SELECT ax_define.bind_enum_table_to_table('${2:column_name}', '${3:table_name}', '${4:app_public}', '${1:enum_name}', '${5:DEFAULT_VALUE}');"
  ],
  "description": [
    "Adds a foreign key to the enum table for a specific column.",
    "If the column does not exist - it is created with type text.\n",
    "The fifth parameter is optional. Do not define if the column already exists or if the new column should not have a default value.",
    "The sixth parameter is optional and decides if the column should be required or optional.",
    "The default value is 'NOT NULL'. Use 'NULL' if the new column should be optional.\n",
    "This is one of the functions that is needed to create an enum table and bind it to a column or migrate from enum type to enum table.",
    "Use 'ax-add-enum-and-column' for a full creation code example.",
    "Use 'ax-migrate-enum-type-to-table' for a full migration code example."
  ]
}
snippet-*/
...
{
  "body": [
      "SELECT ax_define.bind_enum_schema_table_to_table('${3:column_name}', '${4:table_name}', '${5:app_public}', '${1:enum_name}', '${2:enum_schema}', '${6:DEFAULT_VALUE}');"
  ],
  "description": [
...
{
  "body": [
    "SELECT ax_define.set_enum_domain('${2:column_name}', '${3:table_name}', '${4:app_public}', '${1:enum_name}_enum', '${4:app_public}');"
  ],
  "description": [
