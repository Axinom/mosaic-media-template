...
describe('query validatePlaylist', () => {
  ...
  it('returns an error in the GraphQL response if validation throws an error', async () => {
    // Arrange
    vi.spyOn(validation, 'validatePlaylist').mockImplementation(async () => {
  ...
  it('succeeds with issues', async () => {
    ...
    > as unknown as PublishValidationResult<PlaylistPublishedEvent>;
    vi.spyOn(validation, 'validatePlaylist').mockImplementation(
