...
describe('localizationAvailableCheck', () => {
  ...
  it('should set isLocalizationEnabled to false if the service account does not have localization permission', async () => {
    ...
    expect(logger.warn).toHaveBeenCalledWith(
      'The configuration value states that localization should be enabled but the service account does not have permissions for the localization service. Disabling localizations until the service is restarted and the checks are run again.',
    );
  });
